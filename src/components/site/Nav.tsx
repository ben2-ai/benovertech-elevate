    ...
    >
      <nav
        className={`mx-auto flex max-w-6xl items-center justify-between rounded-2xl border px-4 py-2.5 transition-all duration-300 lg:px-6 ${
          scrolled
            ? "border-white/20 bg-[#101a35]/95 shadow-lift backdrop-blur-xl"
            : "border-white/20 bg-[#101a35]/90 backdrop-blur-md"
        }`}
      >
        <Link to="/" className="flex min-w-0 items-center gap-3" onClick={() => setOpen(false)}>
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white p-1.5 shadow-lift ring-1 ring-white/70">
            <img src="/brand-logo.png" alt="BENOVERTECH logo" className="h-full w-full object-contain" />
          </span>
      ...
      {open && (
        <div className="mx-auto mt-2 max-w-6xl rounded-2xl border border-white/20 bg-[#101a35]/95 p-4 shadow-lift backdrop-blur-xl lg:hidden">
